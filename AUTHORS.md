# Authors & Copyright

## 👤 Creator & Sole Copyright Holder

**Samrudh**

- **Role**: Creator, Lead Developer, Architect
- **Project**: Farm Intellect
- **GitHub**: [@samrudh](https://github.com/samrudh)
- **Year**: 2025

---

## 🏛️ Intellectual Property

All intellectual property rights in and to **Farm Intellect**, including but not limited to:

- ✅ All source code and compiled binaries
- ✅ User interface designs and visual assets
- ✅ AI prompts, model configurations, and system architecture
- ✅ Curated agricultural datasets and knowledge bases
- ✅ Documentation, diagrams, and technical specifications
- ✅ Branding, logos, and trademarks ("Farm Intellect")

...are the **exclusive property of Samrudh**.

---

## 📜 License

This project is **proprietary software** licensed under an **All Rights Reserved** license.

See [LICENSE](LICENSE) for full terms and conditions.

**No part of this software may be copied, modified, distributed, or used without prior written authorization from the Author.**

---

## 🙏 Acknowledgments

### Open-Source Dependencies
This project utilizes open-source libraries (React, TypeScript, Tailwind CSS, etc.) under their respective licenses. The use of these libraries does not extend any rights to the proprietary code of this project.

### Data Sources
Agricultural data references include publicly available information from:
- **ICAR** (Indian Council of Agricultural Research)
- **ICAR-CRIDA** (Central Research Institute for Dryland Agriculture)
- **Agmarknet** (Agricultural Marketing Information Network)
- **NCIPM** (National Centre for Integrated Pest Management)
- **Kisan Call Centre** (Government of India)
- **Soil Health Card Scheme** (Ministry of Agriculture & Farmers Welfare)

The specific compilation, curation, and integration of these datasets is proprietary.

---

<p align="center">
  <strong>© 2025 Samrudh. All Rights Reserved.</strong><br/>
  <em>Farm Intellect | AI-Powered Smart Agriculture for India 🇮🇳🌾</em>
</p>
